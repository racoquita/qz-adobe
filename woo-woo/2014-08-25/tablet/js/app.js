var App = function() {
	...
	this.on = function() {
		if (typeof QZIX === 'object') {
			$('.faux-ixtrack').on('click', function (e) {
				var category = $(this).data('ix-category')
				,		label		 = $(this).data('ix-label')
				,		action   = 'click';
				QZIX.manualTrigger(category, action, label, false);
			});			
		}
