QZAD.Shadowboxer = function( settings ) {
      ...
      that.BCLS = ( function() {
        ...
        return {
          onTemplateLoad: function() {
            player = brightcove.api.getExperience('my-experience');
            APIModules = brightcove.api.modules.APIModules;
            videoPlayer = player.getModule( APIModules.VIDEO_PLAYER );
            mediaEvent = brightcove.api.events.MediaEvent;
            videoPlayer.addEventListener( mediaEvent.COMPLETE, that.BCLS.onMediaComplete );
          },
          onMediaComplete: function( e ) {
            that.destroyBrightcoveVideo( true );
          }
        }
      } )();
  ...
  this.destroyBrightcoveVideo = function( fade ) {
    if ( fade ) {
      var posterTimer = setTimeout( function() {
        $('#shadowboxer-wrapper').fadeOut( 500, function() {
          $(this).remove();
